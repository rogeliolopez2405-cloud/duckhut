...
(() => {
  ...
  function difficulty(level=round){const count=level<3?2:level<6?3:4;return {count,ammo:count+1,total:count*5,goal:count*3,speed:Math.min(220,85+level*13),time:Math.max(5.5,8-level*.25)+(count-2)*.65};}
  function hud(){const rules=difficulty(); $('score').textContent=String(score).padStart(6,'0');$('round').textContent=String(round).padStart(2,'0');$('ammo').textContent=Array.from({length:rules.ammo},(_,i)=>i<shots?'●':'○').join(' ');$('hit-count').textContent=`${results.filter(Boolean).length} / ${rules.total}`;$('hit-markers').innerHTML=Array.from({length:rules.total},(_,i)=>`<i class="${i<results.length?(results[i]?'hit':'miss'):''}"></i>`).join('');$('advance-target').textContent=rules.goal+' / '+rules.total;$('hit-markers').setAttribute('aria-label',`${results.filter(Boolean).length} hits, ${results.filter(x=>!x).length} misses`);}
  ...
  function spawn(){const rules=difficulty();dogTime=0;wave++;shots=rules.ammo;timer=rules.time;nextWave=0;ducks=Array.from({length:rules.count},(_,i)=>({x:90+i*(740/rules.count)+Math.random()*50,y:(280+(i%2)*35)*H/540,vx:(Math.random()>.5?1:-1)*rules.speed,vy:-55-Math.min(round,12)*4,phase:Math.random()*6,alive:true,hit:false,done:false,fall:0}));state='playing';hud();toast('ROUND '+round+' · '+rules.count+' DUCKS · WAVE '+wave+' / 5')}
